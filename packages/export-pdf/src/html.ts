...
import { isImageEmbed, isLegacySerializedDocument, pxToMm } from "@editor/core";
 ...
 *
 * Output structure mirrors what `@editor/react` paints at runtime: each
...
function baseStylesheet(page: NonNullable<DocumentToHtmlOptions["page"]>): string {
  ...
  // `@editor/export-pdf-server`). Adding the same offsets via `body { padding }`
